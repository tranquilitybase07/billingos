...
ALTER TABLE feature_grants DROP CONSTRAINT IF EXISTS feature_grants_subscription_id_feature_id_key;

-- Add partial unique index: only one active grant per (subscription_id, feature_id)
CREATE UNIQUE INDEX idx_feature_grants_active_unique
  ON feature_grants (subscription_id, feature_id)
